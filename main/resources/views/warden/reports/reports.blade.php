                    ...
                    <div class="grid grid-cols-1 xl:grid-cols-2 gap-4 md:gap-6">
                        ...
                        <div class="space-y-4">
                            <div class="flex justify-between items-center">
                                ...
                                <span class="font-medium text-gray-900 dark:text-white">{{ $supervisionReports['totalFiles'] }}</span>
                            </div>
                            <div class="border-t border-gray-200 dark:border-gray-700 pt-4">
                                <h4 class="text-sm font-medium text-gray-900 dark:text-white mb-2">Recent Uploads</h4>
                                @if($supervisionReports['recentUploads']->count() > 0)
                                    <div class="space-y-2">
                                        @foreach($supervisionReports['recentUploads'] as $file)
                                            <div class="flex justify-between items-center text-sm">
                                                <span class="text-gray-600 dark:text-gray-400">{{ $file->filename }}</span>
                                                <span class="text-gray-500 dark:text-gray-500">{{ $file->created_at->format('M d, Y') }}</span>
                                            </div>
                                        @endforeach
                                    </div>
                                @else
